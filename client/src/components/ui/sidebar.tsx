...
>(
  (
    ...
    // This is to handle the case where the user manually changes the cookie.
    React.useEffect(() => {
...
>(
  (
    ...
    if (isMobile) {
      return null // We render the mobile sidebar as a sheet in SidebarInset
    }
...
export function AppSidebar({ isCollapsed, onToggle }: AppSidebarProps) {
  ...
  const isSuperAdmin = userInfo?.login === 'admin';
  ...
  const menuItems = [
    ...
    { 
      ...
      show: isSuperAdmin // Só mostra para super admin
    },
  ];

  return (
    <aside className={`fixed left-0 top-16 bottom-0 z-40 bg-white border-r-2 border-blue-200 shadow-lg transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-64'}`}>
